...
def process_srec_file(input_file):
    ...
    aligned_file = os.path.join(SCRIPT_DIR, f"{name_without_ext}_aligned.srec")
    final_file = os.path.join(SCRIPT_DIR, f"{name_without_ext}_aligned_padded.srec")
    ...
    try:
        ...
        print(f"Aligned file: {os.path.basename(aligned_file)}")
...
def main():
    ...
    # List available .srec files from tools directory
    srec_files = list_srec_files(TOOLS_DIR)
    if not srec_files:
